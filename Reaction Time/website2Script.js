...
const experimentState = {
  ...
  const scheduleSound = function () {
    ...
    if (experimentState.times.length >= 5) {audio.volume = 1;}
  };
  ...
  const btnAction = function () {
    ...
    if (!experimentState.started) {startExperiment();} 
    else {
      if (experimentState.ended) {
        const stats = computeStatistics(experimentState.times);
        for(let i = 0; i <experimentState.times.length; i++) { document.querySelector("#time").textContent = "Times: " + experimentState.times + " " ;};
